...
from models.ElectraPretrainingModel import ElectraPretrainingModel
...
tokenizer_special_ids = set(tokenizer.all_special_ids)
...
def tokenize_function(input):
    res = tokenizer(
        input['text'], truncation=True, padding=PaddingStrategy.MAX_LENGTH, max_length=seq_length,
        return_token_type_ids=False
    )
    res['special_tokens_mask'] = [[(1 if t in tokenizer_special_ids else 0) for t in item] for item in res['input_ids']]
    return res
